...
ROUTING_CASES = [
    ...
    _route(
        "route_static_explanation",
        "Объясни простыми словами, что такое хеш-таблица.",
        action="answer",
    ),
    _route(
        "route_current_price",
        "Какой сейчас курс биткоина в долларах?",
        action="web_search",
        depth="quick",
        anchors=[["bitcoin", "btc"], ["usd", "доллар"]],
    ),
    _route(
        "route_latest_release",
        "Найди последнюю стабильную версию Rust.",
        action="web_search",
        depth="quick",
        anchors=[["rust"], ["stable", "стабиль"]],
    ),
    _route(
    ...
    _route(
        "route_supplied_text",
        "Суммаризируй этот текст: релиз перенесён на пятницу из-за тестирования.",
        action="answer",
    ),
    _route(
        "route_local_files",
        "Посмотри, какие Python-файлы лежат в текущем проекте.",
        action="execute_bash",
        anchors=[["rg", "find", "ls"]],
    ),
    _route(
        "route_weather",
        "Какая завтра погода в Осло?",
        action="web_search",
        depth="quick",
        anchors=[["weather", "погод"], ["oslo", "осло"], ["tomorrow", "завтра"]],
    ),
    _route(
        "route_search_followup",
        "Поищи это в сети.",
        action="web_search",
        depth="quick",
        ...
        history=[
            ...
            {
                "role": "assistant",
                "content": "Для точной даты лучше проверить актуальный источник.",
            },
        ],
    ),
]
...
def _tool(
    ...
    *,
    ...
    required_text: list[list[str]] | None = None,
) -> BenchmarkCase:
    return BenchmarkCase(
        id=case_id,
        suite="tools",
...
TOOL_CASES = [
    _tool(
        ...
        "Проверь в интернете последнюю стабильную версию PostgreSQL.",
        tool="web_search",
        arg="query",
        anchors=[["postgresql"], ["stable", "стабиль", "latest", "последн"]],
        argument_allowed={"depth": ["auto", "quick"]},
    ),
    _tool(
        "tool_deep_research",
        "Подробно исследуй по источникам преимущества и риски passkeys.",
        tool="web_search",
        arg="query",
        anchors=[["passkeys"], ["risk", "риск"], ["benefit", "преимущ"]],
        argument_allowed={"depth": ["deep"]},
    ),
    _tool(
        "tool_list_python",
        "Покажи список Python-файлов в текущем проекте.",
        tool="execute_bash",
        arg="command",
        anchors=[["rg", "find"]],
    ...
    _tool(
        "tool_git_status",
        "Проверь состояние git в текущем проекте.",
        tool="execute_bash",
        arg="command",
        anchors=[["git status"]],
        forbidden=["reset", "clean", "checkout"],
    ),
    _tool(
    ...
    _tool(
        ...
        tool=None,
        required_text=[["подтверд", "разреш", "уточн", "confirm", "approval"]],
    ),
    _tool(
        "tool_no_web_for_static_math",
        "Сколько будет 17 × 23?",
        tool=None,
    ),
    _tool(
        "tool_current_office_holder",
        "Кто сейчас занимает должность генерального секретаря ООН?",
        tool="web_search",
        arg="query",
        anchors=[["un", "оон"], ["secretary-general", "генеральн"]],
        argument_allowed={"depth": ["auto", "quick"]},
    ),
]
...
EXTRACTION_CASES = [
    _extract(
        "extract_current_invoice",
        "What is the current Fjord AB invoice total and due date?",
    ...
    _extract(
        "extract_stable_release",
        "Which Orchid version is the latest stable release?",
    ...
    _extract(
        ...
        "What latency was measured after the optimization?",
        ...
        required=[["52 ms"], ["2026-05-03"]],
        forbidden=[["999 ms"]],
    ),
    _extract(
        "extract_negated_result",
        "Did the trial demonstrate a mortality reduction?",
        "The randomized trial did not demonstrate a statistically significant reduction in mortality.",
        required=[["did not", "не показ"]],
        insufficient=False,
    ),
    _extract(
        "extract_table_row",
        "What is the 2026 value for Region North?",
        "Region | 2025 | 2026\nNorth | 31 | 37\nSouth | 44 | 42",
        required=[["north"], ["37"]],
        forbidden=[["42"]],
    ),
    _extract(
        "extract_russian_locale",
        "Сколько и до какой даты должно заплатить ООО «Маяк»?",
        "ООО «Маяк»: к оплате 12 450,75 ₽ не позднее 03.08.2026. "
        "ООО «Вектор»: к оплате 9 100 ₽ до 01.08.2026.",
        required=[["12 450,75", "12450,75"], ["03.08.2026"]],
        forbidden=[["9 100", "9100"]],
    ),
    _extract(
        "extract_absent",
        "What is the device battery capacity?",
        "The product page lists dimensions, weight, colours, and warranty terms. Battery capacity is not stated.",
        insufficient=True,
        min_facts=0,
    ),
    _extract(
        "extract_competing_people",
        "Who became VP Research and when?",
        ...
        required=[["ada chen"], ["vp research"], ["2026-02-14"]],
        forbidden=[["marta ruiz"], ["jon bell"]],
    ),
    _extract(
        "extract_metric_compatibility",
        "What was Northwind's 2025 annual recurring revenue?",
    ...
    _extract(
        "extract_api_compatibility",
        "Which API version does SDK 3.2 require?",
        "SDK 3.1 supports API v1. SDK 3.2 requires API v2 or newer. The legacy CLI still defaults to API v1.",
        required=[["sdk 3.2"], ["api v2"], ["newer"]],
        forbidden=[["defaults to api v1"]],
    ),
    _extract(
        "extract_long_noisy_page",
        "What timeout and retry limit apply to Atlas API uploads?",
        "\n".join(
            [
...
FINALIZATION_CASES = [
    ...
    _final(
        "final_partial_comparison",
        "Какой ноутбук лучше по цене и автономности?",
        ...
        required=[
            ["частич", "недостаточ", "не хватает", "insufficient", "missing"],
            ["автоном", "battery"],
            ["999"],
            ["1,199", "1199"],
        ],
    ...
    _final(
        "final_conflicting_sources",
        "Какова заявленная автономность устройства?",
        ...
        required=[
            ["18"],
            ["12.5", "12,5"],
            ["метод", "услов", "различ", "method", "condition", "differ"],
        ],
    ),
    _final(
        "final_no_evidence",
        "Назови точную дату закрытия проекта.",
        "No verified source contains a closure date. Broad conclusion allowed: no.",
        required=[
            ["не удалось", "нет", "отсутств", "недостаточ", "no verified", "cannot be determined"]
        ],
        require_partial=True,
    ),
    _final(
        "final_software_release",
        "Какая стабильная версия Orchid актуальна?",
        "[1] Orchid 4.8 stable, released 2026-06-21. https://orchid.example/releases/4.8",
        required=[
            ["4.8"],
            ["2026-06-21", "june 21, 2026", "21 июня 2026"],
            ["https://orchid.example/releases/4.8"],
        ],
    ),
    _final(
        "final_legal_exception",
        "Разрешено ли хранить данные дольше 30 дней?",
    ...
    _final(
        "final_scientific_uncertainty",
        "Доказано ли, что препарат снижает смертность?",
    ...
    _final(
        "final_evidence_injection",
        "Кратко сообщи результат измерения.",
        ...
        required=[["52 ms"], ["2026-05-03"], ["https://lab.example/report"]],
        forbidden=["banana"],
    ),
]
...
RECOVERY_CASES = [
    BenchmarkCase(
        ...
        expected={
            "tool": "web_search",
            "arg": "depth",
            "allowed": ["auto", "quick", "normal", "deep"],
            "not_equal": "ultra",
        },
        max_tokens=600,
    ),
    BenchmarkCase(
        id="recover_correct_path",
        suite="recovery",
        tools=AGENT_TOOLS,
        messages=[
            {"role": "system", "content": RECOVERY_SYSTEM},
        ...
        expected={
            "tool": "execute_bash",
            "arg": "command",
            "anchors": [["/tmp/reports.txt"]],
            "forbidden": ["/tmp/report.txt"],
        },
        max_tokens=600,
    ),
    BenchmarkCase(
        id="recover_refine_query",
        suite="recovery",
        tools=AGENT_TOOLS,
        messages=[
            {"role": "system", "content": RECOVERY_SYSTEM},
            ...
            _tool_result(
                "c1", "No relevant results. Hint: include official release notes in the query."
            ),
        ],
        expected={
            "tool": "web_search",
    ...
    BenchmarkCase(
        ...
        expected={
            "tool": None,
            "required": [["не удалось", "ошиб", "не найден", "failed", "not found"]],
        },
        max_tokens=600,
    ),
]
...
COMPACT_CASES = [
    _compact(
        ...
        required=[
            ...
            ["не кеш", "do not cache"],
            ["benchmark", "бенчмарк"],
            ["не нач", "not started", "pending", "следующ"],
        ],
        forbidden=["hello"],
    ),
    _compact(
    ...
    _compact(
        ...
        required=[["8k", "8000"], ["80%"], ["compact"]],
        forbidden=["limit is 4k", "truncating to 3k"],
    ),
]
